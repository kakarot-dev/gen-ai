...
class MCNPCController:
    ...
    def _rl_decide(self, bot_state: dict, target: dict) -> str:
        ...
        h = bot_state.get("health", 20) / 20

        obs = self._build_observation(bot_state, target)
        action, _ = self.rl_model.predict(obs, deterministic=True)
        goal = self._action_to_goal(int(action), bot_state, target)

        z = self.npc_type == "zombie"
        s = self.npc_type == "skeleton"
        f = self._goal_frames

        if h < 0.25:
            goal = "retreat"
        elif h < 0.5 and f % 5 > 2:
            goal = "flank_target"
        elif z and goal in ("ranged_attack", "kite_target", "maintain_distance"):
            goal = "melee_attack"
        elif z and f % 8 == 0 and dist < 10:
            goal = "flank_target"
        elif s and goal in ("melee_attack", "chase_target", "dash_attack") and dist > 8:
            goal = "ranged_attack"
        elif s and f % 6 == 0 and dist < 12:
            goal = "find_vantage_point"
